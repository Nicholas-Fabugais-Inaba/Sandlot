    ...
    async session({ session, token }: { session: Session; token: JWT }) {
      if (token) {
        session.user = {
          ...
          preferred_division: typeof token.preferred_division === 'string' ? token.preferred_division : '',
          preferred_time: typeof token.preferred_time === 'string' ? token.preferred_time : '',
