...
\begin{myreq}
  ...
  (may or may not include name, email, username, password), 
...
\begin{myreq}
  ...
  only more than RESCHED\_NOTICE\_HRS (24 hours) in the future.
  \twoinline
    {\reqsatis 5}
    {\reqdissat 5}
\end{myreq}
...
\begin{myreq}
  ...
  MIN\_INPUT\_SIZE (44x44) pixels, maintain at least a MIN\_CONTRAST\_RATIO
  (4.5:1) contrast ratio with the background.
  \twoinline
    {\reqsatis 2}
    {\reqdissat 4}
\end{myreq}
...
\begin{myreq}
  ...
  the schedule, and it should not take more than MAX\_NAV\_CLICKS (2 clicks)
  to access.
  \twoinline
    {\reqsatis 3}
    {\reqdissat 5}
...
\begin{myreq}
  ...
  their possible actions within MAX\_TRAIN\_MINS (60 minutes).
  \twoinline
    {\reqsatis 3}
    {\reqdissat 2}
\end{myreq}
...
\begin{myreq}
  ...
  possible actions within MAX\_TRAIN\_MINS (60 minutes).
  \twoinline
    {\reqsatis 3}
    {\reqdissat 2}
\end{myreq}
...
\begin{myreq}
  ...
  \reqorig Alex Verity
  \reqfit MIN\_UNDERSTAND\_PERCENT (90 percent) of users understand
  terminology used on the Sandlot user interface.
  \twoinline
    {\reqsatis 3}
    {\reqdissat 2}
\end{myreq}
...
\begin{myreq}
  ...
  \reqorig Alex Verity
  \reqfit Body text shall have a minimum font size of MIN\_FONT\_SIZE (16)
  pixels, and a line length between MIN\_LINE\_CHARS (45) and MAX\_LINE\_CHARS
  (75) characters for optimal readability across all devices.
  \twoinline
    {\reqsatis 3}
    {\reqdissat 4}
\end{myreq}
...
\begin{myreq}
  ...
  a contrast ratio of at least MIN\_CONTRAST\_RATIO (4.5:1).
  \twoinline
    {\reqsatis 2}
    {\reqdissat 4}
\end{myreq}
...
\begin{myreq}
  ...
  \reqorig Alex Verity
  \reqfit The website shall load in under MAX\_LOAD\_SECS (3 seconds) on a
...
\begin{myreq}
  ...
  MIN\_UPTIME\_PERCENT (99 percent) of the time.
  \twoinline
    {\reqsatis 2}
    {\reqdissat 4}
\end{myreq}
...
\begin{myreq}
  ...
  \reqorig Alex Verity
  \reqfit No faults should occur when MAX\_TEAMS (60) teams with
  MAX\_TEAM\_PLAYERS (25) players each are entered into the system.
  \twoinline
    {\reqsatis 2}
    {\reqdissat 2}
\end{myreq}
...
\begin{myreq}
  ...
  \reqorig Alex Verity
  \reqfit No faults should occur when MAX\_USERS (500) users are using the system at once.
  \twoinline
    {\reqsatis 3}
    {\reqdissat 3}
\end{myreq}
...
\begin{myreq}
  ...
  \reqorig Alex Verity
  \reqfit A new season can be started within MAX\_SEASON\_START\_HRS (1 hour).
  \twoinline
    {\reqsatis 5}
    {\reqdissat 5}
\end{myreq}
