...
\begin{myreq}
  ...
  their team's availabilty for the season.
...
\begin{myreq}
  \twoinline
    {\reqno FR-10}
    {\reqevent 6}
  \reqdesc Captains must be able to request a reschedule for their team's
...
\begin{myreq}
  \twoinline
    {\reqno FR-11}
    {\reqevent 3}
  \reqdesc When a captain receives a reschedule request, the system should
  prompt the captain to either accept the reshedule request and choose a date
  ...
  both an option to accept or deny any reshedule request.
  \reqorig Casra Ghazanfari
  \reqfit A captain shall have the option to either accepy or deny any request
...
\begin{myreq}
  \twoinline
    {\reqno FR-12}
    {\reqevent 3}
  \reqdesc When a captains rechedule request is either accepted or denied the
  system should notify them about about the outcome.
  \reqrat It is important that the sender of the rechedule request is informed
  about the status of the request quickly
  \reqorig Casra Ghazanfari
  \reqfit A captain who sent a reschedule request shall be notified on the
...
\begin{myreq}
  ...
  modify the schedule to account for unforseen problems.
  \reqorig Alex Verity
  \reqfit If the commissioner inputs new schedule data, the schedule is
...
\begin{myreq}
  ...
  users.
  \reqrat Commissioners should be assign captains as captains are needed to
...
\begin{myreq}
  ...
  \reqfit The system shall display a shedule that includes all past and
  future games from all teams on a calendar.
  \twoinline
    {\reqsatis 4}
    {\reqdissat 3}
...
\begin{myreq}
  ...
  \reqorig Jung Woo Lee
  \reqfit For each team, the system shall display a shedule that includes all
...
\begin{myreq}
  ...
  \reqrat Sandlot will have a wide variety of users, and we must make sure the
  colours are able to be contrasted by all users.
  \reqorig Jung Woo Lee
...
\begin{myreq}
  \reqno MR-1
  \reqdesc A new season can be started (i.e. saving past standings, reseting
...
Not applicable, however comissioners will support users if they can.
...
\begin{myreq}
  ...
  \reqrat Players may need to contact their captain but but should not have
  access to every account's information.
  \reqorig Nicholas Fabugais-Inaba
  \reqfit Players shall not be able to see any contact information outside of
...
\begin{myreq}
  \reqno AS-5
  \reqdesc Giving permissions to users must be accompanied by an warning that
