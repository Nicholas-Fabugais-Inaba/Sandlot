...
Mar. 3, 2025 & 1.0 & TA Feedback\\
Mar. 10, 2025 & 1.1 & Prior to Rev1\\
Apr. 4, 2025 & 1.2 & Restatement of NFR evaluation criteria.
Based on TA feedback: added explicit references to figures for section 4.\\
Apr. 4 2025 & 1.3 & Based on TA feedback: Explanation of results and what constitutes 
a pass/fail for FR tests 
\bottomrule
\end{tabularx}
...
For all of our FR tests, we had a separate database filled with placeholder data 
that was used to test the system. This means that we at all times knew what the 
database held before, during and after our functional tests. This is how we were
able to determine if our tests passed or failed. Tests that passed matched the 
expected output of the database for a given set of queries while tests that failed 
did not. The actual output of the database for any given test varied greatly 
depending on the queries that were run.


\section{Nonfunctional Requirements Evaluation}

As outlined in the VnV Plan, a Microsoft Form was provided to testers of the system to validate specific
system tests outline in the VnV Plan. Answers for each question ranged from
either, a rating between 1 (Strongly Disagree) to 5 (Strongly Agree), 1 
(Very Difficult) to 5 (Very Easy), or 1 (No, the system was difficult to use 
due to accessibility issues) to 3 (Yes, the system was accessible). A 
satisfactory result for each question should be scored a 3 or above.
...
\begin{enumerate}
  ...
  on their experience. \\
  Average Result: Yes, the system was accessible (Score: 3). Results graphed in \hyperref[visual]{Figure 1}.\\
  ...
  Average Result: Strongly Agree (Score: 5). Results graphed in \hyperref[account]{Figure 2}.\\
  ...
  out and provide their own feedback on their experience. \\
  Average Result: Very/Somewhat Easy (Score: 4.4). Results graphed in \hyperref[nav]{Figure 3}.\\
...
\begin{enumerate}
  ...
  Average Result: Strongly/Slightly Agree (Score: 4.75). Results graphed in \hyperref[access]{Figure 4}.\\
  ...
  Average Result: Slightly Agree (Score: 4). Results graphed in \hyperref[validate]{Figure 5}.\\
...
\begin{longtable}{|c|c|p{8cm}|}
  ...
  \hline
  test\_get\_schedule\_games & Passed & Backend correctly retrieves scheduled 
  game data. \\
  \hline
  test\_team\_games & Passed & Backend correctly retrieves all of the specified 
  team's games. \\
  \hline
  test\_create\_RR & Passed & Backend properly creates a new reschedule request with the 
  specificied data in the database. \\
  \hline
  test\_get\_team\_RRs & Passed & Backend correctly retrieves all reschedule requests adressed to a 
  specific team. \\
  \hline
  test\_delete\_reschedule\_request & Passed &  Backend properly deletes a specified reschedule request 
  from the database. \\
  \hline
  test\_get\_standings\_data & Passed & Backend properly retrieves completed game scores and calculates
  the standings of teams based on them. \\
  \hline
  test\_get\_player\_data & Passed & Backend correctly retrieves the list of players that belong to 
  a specified team including their information. \\
  \hline
  test\_get\_team\_data & Passed & Backend correctly retrieves all teams in the databse and their 
  divisions. \\
  \hline
  test\_create\_player\_account & Passed & Backend properly creates a player account in the database
  with the player information provided to it. \\
  \hline
  test\_create\_team\_account & Passed & Backend properly creates a team account in the database with
  the team information provided to it. \\
  \hline
  test\_get\_player\_account & Passed & Backend correctly retreives a player's account information
  based on the player's email. \\
  \hline
  test\_get\_team\_account & Passed & Backend correctly retrieves a team's account information based on
  the team's username. \\
  \hline
  test\_delete\_player & Passed & Backend properly deletes a player account from the database 
  based on the information provided. \\
  \hline
  test\_delete\_team & Passed & Backend properly deletes a team account from the database based on
...
The system has proven to be fairly robust, with a majority of our unit tests passing successfully. 
This shows its stability, reliability, and ability to perform as expected in these specific areas.
