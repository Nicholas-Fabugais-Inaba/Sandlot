...
\newgeometry{left=0.65in,right=0.25in,top=0.25in,bottom=0.45in}
...
& EU-2 & Misinputted user login information should give a warning to the user when
...
FR-17 & Allow captain to resubmit availability that overwrites previously
...
makes an error. & AC-1 AC-2 & Ensure schedule data and structure is visible and
readable.\\
\cline{2-6}
...
unclear or user makes an error. & AP-1 AP-2 & Ensure that when assigning a team for a
...
makes an error. & AS-12 & Giving an account additional permissions should give
...
their login data. & PV-2 & Remind users to keep their passwords secret andsecure.\\
\cline{1-6}
...
AC-3 & Ensure alert message is visible and readable.\\
\cline{2-6}
\hline
\end{tabularx}
