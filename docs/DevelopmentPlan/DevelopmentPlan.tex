...
We will also have flexible meetings if a specific issue arrises. The meeting
chair will organize an agenda before the meeting. These meetings can be in
...
Call and Text will be used for direct communication between 2 team members
...
the remote branch. Once this is complete, a branch should be created from either the documentation
or development branch for any commits involving documentation, development code, or any other
changes present within the repository. With these commits, there should be comments detailing the
changes that have been made. Once all commits are in place and pushed to the remote branch, a pull
request will be created that will go through our CI/CD process using Github Actions. This will use
automated testing against the development code being submitted. Regarding the pull request for the
branch with all the commit changes, it should be merged to either the documentation or development
branch and the associated Git label should be added (i.e. documentation, bug). At least one
approval will be required from a reviewer for each pull request to merge all the commit changes.
Finally, when the commits have been successfully merged, the changes that were made on the
development or documentation branch will then be merged to main as the main branch will act as
the team's "production" branch. Main will be merged and updated with for milestones, POC's, and
final deployments.
\\
\\
...
\\
\\
\indent For each issue, a team member should be assigned to the issue on Github. The descriptions
...
\\
\\
\indent Milestones and checklists will be used to help keep the team on track. The team will
...
The team will participate in a group crosswords when members are able to find the time.
The team will also participate in sports days when possible.
