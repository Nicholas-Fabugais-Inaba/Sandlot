...
Test reschedule with different dates.
...
Test scheduler with different season start and end dates.
					
\item{test-id2\\}

Test scheduler with different number of games per team.

\item{test-id3\\}

Test scheduler with different numbers of divisions and teams per division.

\item{test-id4\\}

Test scheduler with different team offdays and perferred times.
...
system tests outlined in the VnV plan. Answers for each question will range from a
rating between 1 (Strongly Disagree) to 5 (Strongly Agree). A satisfactory result for
each question should be a 3 (Neutral) and above. Additionally, the team should address
any issues the user has with the system, which will be received from question 6 of the survey.
\\\\Questions in the Google Form will be the following:
